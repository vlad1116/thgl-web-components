...
export function CoordinatesProvider({
    ...
    actorLoop: for (const actor of actors) {
      ...
      if (staticNode && staticNode.spawns.length && !debug) {
        for (let i = 0; i < staticNode.spawns.length; i++) {
          ...
          if (dx > -PROXIMITY_THRESHOLD && dx < PROXIMITY_THRESHOLD &&
              dy > -PROXIMITY_THRESHOLD && dy < PROXIMITY_THRESHOLD) {
  ...
  }, [isHydrated, liveMode, actors, customNodes, staticNodes, typesIdMap]);
    ...
    actors.forEach((actor) => {
      ...
      if (staticNode) {
        const spawn = staticNode.spawns.find((spawn) => {
          const dx = spawn.p[0] - actor.x;
          const dy = spawn.p[1] - actor.y;
          return dx > -PROXIMITY_THRESHOLD && dx < PROXIMITY_THRESHOLD &&
                 dy > -PROXIMITY_THRESHOLD && dy < PROXIMITY_THRESHOLD;
        });
        if (spawn) {
          const nodeId = getNodeId(spawn.id, staticNode.type, spawn.p[0], spawn.p[1]);
  ...
  }, [typesIdMap, actors, filters, staticNodes]);
  ...
  const refreshSpawns = useCallback(
    ...
    [nodes, searchIndex, publicSearchSpawns, globalFilters],
  );
