  ...
  const menu = data.map((item) => {
    return {
      ...
      items: item.items.map((subitem) => ({
        ...
        href: localizePath(`/db/items/${subitem.id}`, locale),
        subtitle: subitem.groupId ? resolveDictWithFallback(enDict, `ui.slot_${subitem.groupId}`, subitem.groupId) : undefined,
      })),
    };
  });
  ...
  return (
    <HeaderOffset full>
      <ContentLayout
        ...
        header={
          <>
            <h2 className="text-2xl">{resolveDict(enDict, "ui.artefacts")}</h2>
          </>
        }
