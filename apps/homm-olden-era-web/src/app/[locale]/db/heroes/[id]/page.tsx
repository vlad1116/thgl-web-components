...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {  const { id, locale = "en" } = await params;  return generateEntryMetadata(locale, "heroes", id);}
    ...
    <HeaderOffset full>
      <ContentLayout
        ...
        header={
          <Breadcrumb
            crumbs={[
              { label: sectionLabel, href: "/db/heroes" },
              { label: entryLabel },
            ]}
            locale={locale}
            dict={dict}
